...
python example/manage.py test --with-xunit --with-xcover --cover-package=pretty_times
TEST_EXIT=$?
rm -rf jenkins_reports
mkdir jenkins_reports
pep8 pretty_times > jenkins_reports/pep8.report
PEP8_EXIT=$?
pyflakes pretty_times > jenkins_reports/pyflakes.report
PYFLAKES_EXIT=$?
